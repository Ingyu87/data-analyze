...
export const getAIPrincipleExplanation = (step) => {
  const explanations = {
    'file-upload': {
      ...
      principle: '패턴 찾기 (Pattern Recognition)',
...
이렇게 파일에서 데이터를 찾아내는 것을 "패턴 찾기"라고 해요. 컴퓨터가 반복해서 연습해서 배운 거예요!`,
      icon: '🔍',
    ...
    'data-parsing': {
      ...
      principle: '구조 알아보기 (Structure Recognition)',
...
컴퓨터가 파일의 구조(어디가 제목이고, 어디가 데이터인지)를 알아채서, 숫자와 글자를 제대로 구분하는 것을 "구조 알아보기"라고 해요. 패턴을 찾아서 데이터를 정리하는 거예요!`,
      icon: '📝',
    ...
    'graph-visualization': {
      ...
      principle: '숫자를 그림으로 바꾸기 (Data Visualization)',
      explanation: `숫자들을 보기 좋은 그래프로 바꾸는 거예요!
      
컴퓨터가 숫자들을 받아서 "이 숫자는 높게, 이 숫자는 낮게"라고 계산해서 그래프를 그려요. 마치 우리가 점들을 찍어서 선으로 연결하는 것처럼요!

꺾은선 그래프는 숫자들이 어떻게 변하는지 보여주고, 막대 그래프는 각 숫자가 얼마나 큰지 비교해서 보여줘요. 이렇게 숫자를 그림으로 바꾸는 것을 "숫자를 그림으로 바꾸기"라고 해요!`,
      icon: '📈',
    ...
    'trend-analysis': {
      ...
      principle: '규칙 찾기 (Linear Regression)',
...
예를 들어, 1월에 10개, 2월에 12개, 3월에 14개라면, 컴퓨터가 "매달 2개씩 늘어나고 있네!"라고 계산하는 거예요. 이렇게 패턴을 찾는 것을 "규칙 찾기"라고 해요. 마치 우리가 수학 문제에서 규칙을 찾는 것처럼요!`,
      icon: '🔎',
    ...
    'correlation-analysis': {
      ...
      principle: '친구 찾기 (Correlation Analysis)',
...
이렇게 두 데이터의 관계를 찾는 것을 "친구 찾기"라고 해요. 마치 두 친구가 함께 다니는지, 아니면 서로 반대로 다니는지 확인하는 것처럼요!`,
      icon: '🤝',
    ...
    'ai-explanation': {
      ...
      principle: '쉬운 말로 바꾸기 (Natural Language Generation)',
...
이렇게 숫자를 우리가 이해하기 쉬운 말로 바꾸는 것을 "쉬운 말로 바꾸기"라고 해요. 마치 번역기처럼, 숫자 언어를 우리 말로 바꿔주는 거예요!
...
컴퓨터가 많은 책과 글을 읽어서 배운 거예요. 그래서 우리가 이해하기 쉽게 설명할 수 있는 거죠!`,
    ...
    'prediction': {
      ...
      principle: '앞으로 어떻게 될까? (Predictive Modeling)',
...
컴퓨터가 과거의 패턴을 학습해서, 그 패턴이 계속되면 미래에 어떻게 될지 계산하는 거예요. 이렇게 미래를 예측하는 것을 "앞으로 어떻게 될까?"라고 해요.
...
10년 후, 20년 후도 같은 방법으로 예측할 수 있어요. 하지만 시간이 길수록 정확도는 낮아질 수 있어요!`,
    ...
    'question-generation': {
      ...
      principle: '맞춤 문제 만들기 (Adaptive Learning)',
...
각 그래프마다 다른 문제가 만들어져요. 왜냐하면 그래프의 내용이 다르기 때문이에요. 이렇게 데이터에 맞춰서 문제를 자동으로 만드는 것을 "맞춤 문제 만들기"라고 해요.
...
컴퓨터가 학생이 어떤 그래프를 보고 있는지 이해하고, 그에 맞는 문제를 만들어주는 거예요!`,
    ...
    'grading': {
      ...
      principle: '자동 채점 (Automated Assessment)',
      explanation: `학생이 푼 문제를 자동으로 채점해주는 거예요!
      
컴퓨터가 정답과 학생의 답을 비교해서 "맞았어요!" 또는 "틀렸어요!"라고 알려주는 거예요. 그리고 왜 맞았는지, 왜 틀렸는지 설명도 해줘요.

마치 선생님이 숙제를 채점하시는 것처럼, 컴퓨터가 빠르고 정확하게 채점해주는 거예요. 이렇게 자동으로 채점하는 것을 "자동 채점"이라고 해요.
