...
이렇게 파일에서 데이터를 찾아내는 것을 "패턴 인식"이라고 해요. 컴퓨터가 반복해서 연습해서 배운 거예요!`,
      icon: '🔍',
      example: '엑셀 파일에서 숫자와 글자를 구분해서 읽어요.'
    },
    
    'data-parsing': {
      title: '🔢 2단계: 데이터 정리 - 자연어 처리 AI',
      principle: '자연어 처리 (Natural Language Processing)',
      explanation: `파일에서 읽은 숫자들을 컴퓨터가 이해할 수 있도록 정리하는 거예요!
      
예를 들어, "1월: 10개, 2월: 15개"라는 글을 읽으면, 컴퓨터가 "1월은 10, 2월은 15구나!"라고 알아채는 거예요. 마치 우리가 일기장을 읽으면서 중요한 내용을 찾아내는 것처럼요!

이렇게 글자와 숫자를 이해하고 정리하는 것을 "자연어 처리"라고 해요. 컴퓨터가 우리 말을 이해하는 방법이에요!`,
      icon: '📝',
      example: 'CSV 파일의 "이름, 숫자" 형식을 이해해서 데이터로 변환해요.'
    },
    
    'graph-visualization': {
      title: '📊 3단계: 그래프 만들기 - 데이터 시각화',
...
꺾은선 그래프는 숫자들이 어떻게 변하는지 보여주고, 막대 그래프는 각 숫자가 얼마나 큰지 비교해서 보여줘요. 이렇게 숫자를 그림으로 바꾸는 것을 "데이터 시각화"라고 해요!`,
      icon: '📈',
      example: '10, 15, 20, 25라는 숫자를 받아서 그래프로 그려요.'
    },
    
    'trend-analysis': {
      title: '📈 4단계: 패턴 찾기 - 선형 회귀 AI',
...
예를 들어, 1월에 10개, 2월에 12개, 3월에 14개라면, 컴퓨터가 "매달 2개씩 늘어나고 있네!"라고 계산하는 거예요. 이렇게 패턴을 찾는 것을 "선형 회귀"라고 해요. 마치 우리가 규칙을 찾는 것처럼요!`,
      icon: '🔎',
      example: '그래프의 점들을 보고 "매달 2씩 증가한다"는 패턴을 찾아요.'
    },
    
    'correlation-analysis': {
      title: '🔗 5단계: 관계 찾기 - 상관관계 분석 AI',
...
이렇게 두 데이터의 관계를 찾는 것을 "상관관계 분석"이라고 해요. 마치 두 친구가 함께 다니는지, 아니면 서로 반대로 다니는지 확인하는 것처럼요!`,
      icon: '🤝',
      example: '온도 데이터와 감기 환자 데이터를 비교해서 관계를 찾아요.'
    },
    
    'ai-explanation': {
      title: '🤖 6단계: 쉬운 설명 만들기 - 자연어 생성 AI',
...
이렇게 숫자를 우리가 이해하기 쉬운 말로 바꾸는 것을 "자연어 생성"이라고 해요. 마치 번역기처럼, 숫자 언어를 우리 말로 바꿔주는 거예요!
...
컴퓨터가 과거의 패턴을 학습해서, 그 패턴이 계속되면 미래에 어떻게 될지 계산하는 거예요. 이렇게 미래를 예측하는 것을 "예측 모델링"이라고 해요.
...
각 그래프마다 다른 문제가 만들어져요. 왜냐하면 그래프의 내용이 다르기 때문이에요. 이렇게 데이터에 맞춰서 문제를 자동으로 만드는 것을 "적응형 학습"이라고 해요.
...
마치 선생님이 숙제를 채점하시는 것처럼, 컴퓨터가 빠르고 정확하게 채점해주는 거예요. 이렇게 자동으로 채점하는 것을 "자동 평가"라고 해요.
