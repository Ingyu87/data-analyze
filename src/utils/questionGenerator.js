...
export const generateQuestions = (data) => {
  ...
  questions.push({
    ...
    points: 20,
    achievementStandard: '[4수04-02]',
    explanation: `그래프의 선이 오른쪽으로 갈수록 올라가면 값이 커지는 것이고, 내려가면 값이 작아지는 것이에요. 이 그래프는 ${trendResult === '증가' ? '증가' : trendResult === '감소' ? '감소' : '변화가 거의 없는'} 경향을 보이고 있어요.`
  });

  // 문제 4: 그래프 선택하기 (4학년 수준)
  questions.push({
    id: 4,
    type: 'graph-interpretation',
    category: '그래프 해석',
    question: `${title || '이 데이터'}를 나타낼 때 막대그래프와 꺾은선그래프 중 어떤 그래프가 더 적합한가요?`,
    options: [
      '꺾은선그래프 - 시간에 따른 변화를 보기 좋아요',
      '막대그래프 - 각 항목을 비교하기 좋아요',
      '둘 다 적합해요',
      '그래프로 나타낼 수 없어요'
    ],
    correctAnswer: 0,
    ...
    explanation: `시간에 따른 변화를 보여주는 데이터는 꺾은선그래프가 더 적합해요. 꺾은선그래프는 시간이 지나면서 값이 어떻게 변하는지 한눈에 볼 수 있기 때문이에요. 막대그래프는 각 항목을 비교할 때 주로 사용해요.`
  });

  // 문제 5: 미래 예측하기 (4학년 수준)
  if (nextVal !== undefined) {
    const nextValRounded = Math.round(nextVal * 10) / 10;
    
    questions.push({
      id: 5,
      type: 'graph-interpretation',
      category: '그래프 해석',
      question: `지금처럼 계속 변한다면 다음 값은 대략 얼마가 될까요?`,
      options: [
        `약 ${nextValRounded}`,
        `약 ${Math.round(nextVal)}`,
        `약 ${nextValRounded + 5}`,
        '예측할 수 없어요'
      ],
      correctAnswer: 0,
      points: 20,
      achievementStandard: '[4수04-02]',
      explanation: `그래프의 변화 경향을 보면, 지금처럼 계속 변한다면 다음 값은 약 ${nextValRounded} 정도가 될 것으로 예상해요. 그래프의 선이 계속 이어지는 모습을 상상해보면 알 수 있어요.`
    });
  }
