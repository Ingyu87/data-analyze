...
- 🤖 **AI 기반 설명**: Google Gemini를 활용한 초등학생용 친절한 설명 생성
...
- **Google Gemini** - AI 기반 설명 생성 ([AI Studio](https://aistudio.google.com/apikey)에서 API 키 발급)
...
5. **환경 변수 설정** (중요!):
   ...
   - **Name**: `GEMINI_API_KEY`
   - **Value**: Gemini API 키 ([Google AI Studio](https://aistudio.google.com/apikey))
   - **Environment**: Production, Preview, Development 모두 선택
   - (선택) **Name**: `GEMINI_MODEL` — 기본값 `gemini-2.0-flash` 대신 사용할 모델 ID (예: `gemini-1.5-flash`)
   - **Name**: `KOSIS_API_KEY`
...
3. 다음 환경 변수 추가:
   ...
   - **Value**: Gemini API 키
   - **Environment**: Production, Preview, Development 모두 선택
   - (선택) **Name**: `GEMINI_MODEL`, **Value**: 예) `gemini-1.5-flash`
   - **Name**: `KOSIS_API_KEY`
   - **Value**: `your_kosis_api_key_here` (KOSIS API 키 입력)
   - **Environment**: Production, Preview, Development 모두 선택
