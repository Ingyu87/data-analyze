 ...
 * Google Gemini API를 사용하여 초등학생용 설명을 생성합니다.
 * API 키는 GEMINI_API_KEY(Vercel 환경 변수)로만 주입합니다.
 */
  ...
  "futurePredictionEvidence": ["예측 근거1", "예측 근거2"]
    ...
    const geminiBody = {
      systemInstruction: {
        parts: [
          {
            text: '당신은 초등학생을 위한 친절하고 이해하기 쉬운 데이터 설명 전문가입니다. 사용자가 요청한 스키마에 맞는 유효한 JSON만 응답하세요. 마크다운 코드 블록이나 JSON 바깥의 부가 설명을 붙이지 마세요.',
          },
        ],
      },
      contents: [
    ...
    try {
      ...
      console.error('Gemini response JSON parse error:', e);
      return res.status(502).json({ error: 'AI 응답 파싱 실패', details: rawText.substring(0, 500) });
    }
    ...
    const aiResponse = candidate?.content?.parts?.map((p) => p.text || '').join('') || '';
    ...
    const tryParseAiJson = (text) => {
      ...
      const candidateStr = fence ? fence[1].trim() : trimmed;
      return JSON.parse(candidateStr);
    };

    try {
      const parsed = tryParseAiJson(aiResponse);
